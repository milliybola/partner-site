...
const ShiftPage: React.FC = () => {
  ...
  return (
    <div className="space-y-8 font-Outfit">
      ...
      {userRole === 'superadmin' && (
        <div className="flex bg-slate-950/60 p-1 rounded-xl border border-edge max-w-md">
          <button
        ...
        <div className="space-y-6">
          {loadingCurrent ? (
            ...
            <div className="space-y-6">
              ...
              <div className="p-6 rounded-2xl bg-darkCard border border-edge shadow-xl flex flex-col md:flex-row md:items-center justify-between gap-6 relative overflow-hidden">
                ...
                <div className="flex items-start gap-4">
                  ...
                  <div className="space-y-1.5 text-left">
                    <div className="flex items-center gap-3">
                      <h2 className="text-xl font-bold text-ink">Smena #{currentShift.id}</h2>
                    ...
                    <div className="flex flex-col sm:flex-row sm:items-center gap-x-6 gap-y-1 text-xs text-slate-400">
                      <span className="flex items-center gap-1"><User className="w-3.5 h-3.5 text-brand" /> Ochgan xodim: <strong className="text-ink font-medium">{currentShift.opened_by_name}</strong></span>
                      <span className="flex items-center gap-1"><Calendar className="w-3.5 h-3.5 text-brand" /> Ochilgan vaqt: <strong className="text-ink font-medium">{formatDate(currentShift.opened_at)}</strong></span>
                    </div>
                  </div>
                </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                ...
                <div className="p-6 rounded-2xl bg-darkCard border border-edge relative overflow-hidden group hover:border-brand/20 transition-all duration-300 text-left">
                  ...
                  <h3 className="text-2xl font-bold text-ink mt-1 truncate">{formatUzS(currentShift.expected_closing_balance)}</h3>
                </div>

                {/* Total Orders Count */}
                <div className="p-6 rounded-2xl bg-darkCard border border-edge relative overflow-hidden group hover:border-violet-500/20 transition-all duration-300 text-left">
                  ...
                  <h3 className="text-2xl font-bold text-ink mt-1">{currentShift.total_orders_count} ta</h3>
                </div>

                {/* Total revenue */}
                <div className="p-6 rounded-2xl bg-darkCard border border-edge relative overflow-hidden group hover:border-emerald-500/20 transition-all duration-300 text-left">
                  ...
                  <h3 className="text-2xl font-bold text-ink mt-1 truncate">{formatUzS(currentShift.total_revenue)}</h3>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 text-left">
                ...
                <div className="p-6 rounded-2xl bg-darkCard border border-edge space-y-5">
                  <div className="flex items-center gap-2 border-b border-edge pb-3">
                    <Coins className="w-5 h-5 text-brand" />
                    <h3 className="font-bold text-ink text-base">To'lov turlari bo'yicha</h3>
                  </div>

                  <div className="space-y-4">
                    <div className="flex items-center justify-between p-3 rounded-xl bg-slate-900 border border-edge">
                      <div className="flex items-center gap-2">
                        <span className="w-2.5 h-2.5 rounded-full bg-emerald-400" />
                        <span className="text-xs font-medium text-slate-300">Naqd pul tushumi (Cash)</span>
                      </div>
                      <span className="font-bold text-ink text-sm">{formatUzS(currentShift.cash_revenue)}</span>
                    </div>
                    ...
                    <div className="flex items-center justify-between p-3 rounded-xl bg-slate-900 border border-edge">
                      ...
                      <span className="font-bold text-ink text-sm">{formatUzS(currentShift.card_revenue)}</span>
                    </div>
                    
                    <div className="pt-2 border-t border-edge flex justify-between items-center text-xs font-bold text-slate-400">
                      <span>Boshlang'ich kassa qoldig'i:</span>
                      <span className="text-ink">{formatUzS(currentShift.opening_balance)}</span>
                    </div>
                  </div>
                </div>
                ...
                <div className="p-6 rounded-2xl bg-darkCard border border-edge space-y-5">
                  <div className="flex items-center gap-2 border-b border-edge pb-3">
                    ...
                    <h3 className="font-bold text-ink text-base">Xizmat turlari bo'yicha</h3>
                  </div>

                  <div className="space-y-3.5">
                    <div className="flex justify-between items-center text-xs pb-2 border-b border-edge text-slate-400">
                      <span className="flex items-center gap-1.5"><Truck className="w-4 h-4 text-violet-400" /> Yetkazib berish (Delivery)</span>
                      <span className="font-bold text-ink">{formatUzS(currentShift.delivery_revenue)}</span>
                    </div>

                    <div className="flex justify-between items-center text-xs pb-2 border-b border-edge text-slate-400">
                      <span className="flex items-center gap-1.5"><Building className="w-4 h-4 text-amber-400" /> Zalda tanovul qilish (Dine-in)</span>
                      <span className="font-bold text-ink">{formatUzS(currentShift.dine_in_revenue)}</span>
                    </div>

                    <div className="flex justify-between items-center text-xs text-slate-400">
                      <span className="flex items-center gap-1.5"><User className="w-4 h-4 text-cyan-400" /> Olib ketish (Pickup)</span>
                      <span className="font-bold text-ink">{formatUzS(currentShift.pickup_revenue)}</span>
                    </div>
                  </div>
                </div>
              </div>
            ...
            <div className="max-w-md mx-auto p-8 rounded-2xl bg-darkCard border border-edge shadow-2xl relative overflow-hidden group hover:border-brand/20 transition-all duration-300">
              <div className="absolute top-0 right-0 w-24 h-24 bg-brand/5 rounded-bl-full pointer-events-none" />
              ...
              <div className="text-center space-y-3 mb-6">
                ...
                <h3 className="text-xl font-bold text-ink">Yangi Smena Ochish</h3>
                ...
                <div className="space-y-1.5">
                  ...
                  <div className="relative">
                    <input
                      ...
                      className="w-full bg-slate-900 border border-edge focus:border-brand rounded-xl pl-4 pr-16 py-3 text-sm font-bold text-ink placeholder-slate-500 focus:outline-none transition"
      ...
      {activeTab === 'history' && userRole === 'superadmin' && (
        <div className="space-y-6 text-left">
          ...
          {loadingHistory ? (
            ...
            <div className="flex flex-col items-center justify-center py-20 text-rose-400 bg-rose-500/5 border border-rose-500/10 rounded-2xl p-6 text-center">
              <AlertCircle className="w-12 h-12 mb-3 animate-pulse" />
              <h4 className="font-bold text-ink text-base mb-1">Yuklashda xatolik</h4>
                ...
                <table className="w-full text-left border-collapse text-sm">
                  ...
                  <tbody className="divide-y divide-edge text-slate-300">
                    {historyShifts.map((shift) => {
                      ...
                      return (
                        <tr key={shift.uuid} className="hover:bg-overlay transition-colors">
                          <td className="py-4 pl-2 font-bold text-ink">
                            #{shift.id}
                          </td>
                          ...
                          <td className="py-4 text-right font-mono font-bold text-xs">
                            {!shift.closed_at ? (
                              ...
                              <span className="text-slate-400 bg-overlay border border-edge px-2 py-0.5 rounded-lg">
                                Balans
                              </span>
                            )}
                          </td>
            ...
            <div className="flex flex-col items-center justify-center py-24 text-slate-500 bg-darkCard/50 border border-dashed border-edge rounded-2xl">
              ...
              <h3 className="font-bold text-ink mb-1">Hisobotlar mavjud emas</h3>
      ...
      {isCloseModalOpen && currentShift && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/75 backdrop-blur-sm p-4 animate-[fadeIn_0.2s_ease-out]">
          <div className="w-full max-w-md bg-darkCard border border-edge-strong rounded-2xl shadow-2xl p-6 relative flex flex-col text-left space-y-5 animate-[slideUp_0.3s_ease-out]">
            {/* Header */}
            ...
            <form onSubmit={handleCloseShiftSubmit} className="space-y-4">
              ...
              {/* Buttons */}
              <div className="flex gap-3 justify-end pt-3 border-t border-edge mt-4">
                <button
                  type="button"
                  onClick={() => setIsCloseModalOpen(false)}
      ...
      {closedShiftResult && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/85 backdrop-blur-md p-4 animate-[fadeIn_0.2s_ease-out]">
          <div className="w-full max-w-lg bg-darkCard border border-edge-strong rounded-2xl shadow-2xl p-6 relative flex flex-col text-left space-y-5 animate-[slideUp_0.3s_ease-out]">
            {/* Logo and title */}
            <div className="text-center space-y-2 border-b border-edge pb-4">
              ...
              <h2 className="text-2xl font-bold text-ink tracking-tight">Smena Muvaffaqiyatli Yopildi</h2>
            ...
            {(() => {
              ...
              return (
                <div className="space-y-4">
                  ...
                  {diff < 0 ? (
                    <div className="p-4 rounded-xl bg-rose-500/10 border border-rose-500/20 text-rose-400 text-xs flex gap-3 items-start">
                      ...
                      <div className="space-y-1">
                        <strong className="font-bold text-ink text-sm block">Kassada Kamomad (Shortage)</strong>
                    ...
                    <div className="p-4 rounded-xl bg-emerald-500/10 border border-emerald-500/20 text-emerald-400 text-xs flex gap-3 items-start">
                      ...
                      <div className="space-y-1">
                        <strong className="font-bold text-ink text-sm block">Kassada Ortiqcha (Surplus)</strong>
                        <p className="leading-relaxed text-slate-400 font-semibold">
                    ...
                    <div className="p-4 rounded-xl bg-overlay border border-edge-strong text-slate-300 text-xs flex gap-3 items-start">
                      <CheckCircle className="w-5 h-5 shrink-0 mt-0.5 text-emerald-400" />
                      <div className="space-y-1">
                        <strong className="font-bold text-ink text-sm block">Kassa Balansi To'g'ri (Balanced)</strong>
                        <p className="leading-relaxed text-slate-400 font-semibold">
                  ...
                  {/* Stat breakdown list */}
                  <div className="p-4.5 rounded-xl bg-slate-900 border border-edge space-y-3.5 text-xs">
                    <div className="flex justify-between items-center py-1 border-b border-edge">
                      ...
                      <span className="font-bold text-ink">{closedShiftResult.closed_by_name || closedShiftResult.opened_by_name}</span>
                    </div>
                    <div className="flex justify-between items-center py-1 border-b border-edge">
                      <span className="text-slate-400 font-semibold">Yopilgan vaqt:</span>
                      <span className="font-bold text-ink">{formatDate(closedShiftResult.closed_at)}</span>
                    </div>
                    <div className="flex justify-between items-center py-1 border-b border-edge">
                      <span className="text-slate-400 font-semibold">Boshlang'ich kassa qoldig'i:</span>
                      <span className="font-bold text-ink">{formatUzS(closedShiftResult.opening_balance)}</span>
                    </div>
                    <div className="flex justify-between items-center py-1 border-b border-edge">
                      <span className="text-slate-400 font-semibold">Kutilgan yakuniy qoldiq:</span>
                      <span className="font-bold text-ink">{formatUzS(closedShiftResult.expected_closing_balance)}</span>
                    </div>
                    <div className="flex justify-between items-center py-1 border-b border-edge">
                      <span className="text-slate-400 font-semibold">Kiritilgan haqiqiy qoldiq:</span>
                      <span className="font-bold text-ink">{formatUzS(closedShiftResult.closing_balance)}</span>
                    </div>
            ...
            <div className="pt-4 border-t border-edge flex flex-col sm:flex-row items-center justify-between gap-4">
              {requiresLogout && countdown !== null ? (
                <div className="flex items-center gap-2 text-xs font-semibold text-rose-400 bg-rose-500/10 px-3 py-1.5 rounded-lg border border-rose-500/20">
                  ...
                  <span>Tizimdan avtomatik chiqish: <strong className="text-sm font-bold text-ink">{countdown} s</strong></span>
                </div>
              ) : (
              ...
              <button
                ...
                className={`w-full sm:w-auto px-6 py-3 rounded-xl font-bold text-sm shadow-md transition-all duration-200 cursor-pointer flex items-center justify-center gap-2 hover:scale-[1.02] active:scale-[0.98] ${
                  requiresLogout 
                    ...
                    : 'bg-slate-800 hover:bg-slate-700 text-slate-200 border border-edge'
                }`}
              >
                {requiresLogout ? (
                  <>
